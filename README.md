# Hi there 👋 I'm Hana Ayman

🎓 Computer Science student (Artificial Intelligence Major)  
🤖 Passionate about Machine Learning, Deep Learning & Computer Vision  
💻 Building real-world AI and web applications using Python & React.js  
📊 Interested in Data Science and intelligent systems  

---

## 🚀 About Me
- 🔭 Currently working on AI/ML and Full-Stack projects  
- 🌱 Learning advanced Deep Learning and Computer Vision techniques
- 🎯 My goal to become an AI/ML Engineer and build impactful intelligent systems  
- 🏐 Professional Volleyball Player (Smouha SC & Egypt National Team)  

---

## 🧠 Tech Stack
- **AI/ML:** Python, TensorFlow, PyTorch, Scikit-learn  
- **Computer Vision:** OpenCV, YOLO  
- **Frontend:** React.js, JavaScript, HTML, CSS  
- **Data:** Pandas, NumPy, Matplotlib  
- **Tools:** Git, GitHub, Docker  

---

## 📌 Featured Projects
- 🧠 Deep Learning models (CNNs for image classification & segmentation)  
- 👁️ Computer Vision systems using OpenCV  
- 🌐 React.js web applications with API integration  

---

## 📫 Connect with Me
- 💼 LinkedIn: [www.linkedin.com/in/hana-ayman-ibrahim]  
- 📧 Email: haymam8@gmail.com  
- 🌍 Location: Alexandria, Egypt  

---

⭐ Always open to internships and collaboration opportunities in AI/ML & Software Development
